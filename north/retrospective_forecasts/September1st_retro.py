    ...
    for year in range(1979,fmax+1):
        if (year == ymax) or (year == ymax-1):
            if len(glob.glob(home+'/DATA/nt_'+str(year)+'08*nrt_n.bin'))==0:
                for day in range(1,31+1):
                    with closing(request.urlopen(sic_ftp1+'/nt_'+str(year)+'08'+str('%02d'%day)+'_f18_nrt_n.bin')) as r:
                        with open(home+'/DATA/nt_'+str(year)+'08'+str('%02d'%day)+'_f18_nrt_n.bin', 'wb') as f:
                            shutil.copyfileobj(r, f)
                    
            files = sorted(glob.glob(home+'/DATA/nt_'+str(year)+'08'+'*nrt_n.bin'))
            ...
            if year < 1988:
                ...
                sat = 'f17'
            files = glob.glob(home+'/DATA/nt_'+str(year)+'08*.1_n.bin')
            if len(files) == 0:
                with closing(request.urlopen(sic_ftp2+'/nt_'+str(year)+'08_'+sat+'_v1.1_n.bin')) as r:
                    with open(home+'/DATA/nt_'+str(year)+'08_'+sat+'_v1.1_n.bin', 'wb') as f:
                            shutil.copyfileobj(r, f)
            icefile = open(glob.glob(home+'/DATA/nt_'+str(year)+'08*.1_n.bin')[0], 'rb')
            contents = icefile.read()
            icefile.close()
            s="%dB" % (int(dimX*dimY),)
            z=struct.unpack_from(s, contents, offset = 300)
...
def forecast(fmin,fmax):
    ...
    GPR = {}
    l_init = [np.logspace(-7,2,20)[8],np.logspace(-7,2,20)[9],np.logspace(-7,2,20)[3]]
    sigma_init = [np.logspace(-3,9,20)[6],np.logspace(-3,9,20)[3],np.logspace(-3,9,20)[13]]
    for k in range(3):
...
df_dt.to_csv(home+'/September1st_detrended_forecasts_'+str(fmin)+'-'+str(fmax)+'.csv')
df_rt.to_csv(home+'/September1st_forecasts_with_trend_'+str(fmin)+'-'+str(fmax)+'.csv')
