...
def readNSIDC(ymax):
    ...
    #NRT data#
    if os.path.exists('DATA/NSIDC0081_SEAICE_PS_N25km_'+str(ymax)+'0630_v2.0.nc') == False:
        os.system("sed -i '' 's/DATEMIN/"+str(ymax)+"-06-01/g' nsidc_download_nrt.py")
        os.system("sed -i '' 's/DATEMAX/"+str(ymax)+"-06-30/g' nsidc_download_nrt.py")
        os.system("python nsidc_download_nrt.py")
        while os.path.exists('NSIDC0081_SEAICE_PS_N25km_'+str(ymax)+'0630_v2.0.nc')==False:
            pass
        os.system('rm NSIDC0081*.xml')
        os.system('rm NSIDC0081*.png')
        os.system('mv NSIDC0081*.nc DATA')
        os.system("sed -i '' 's/"+str(ymax)+"-06-01/DATEMIN/g' nsidc_download_nrt.py")
        os.system("sed -i '' 's/"+str(ymax)+"-06-30/DATEMAX/g' nsidc_download_nrt.py")
            
    NRT = xr.open_mfdataset('DATA/NSIDC0081*.nc',concat_dim='time',combine='nested').F18_ICECON
    NRT = NRT.where(NRT<=1).mean('time').to_numpy()
            
    #MONTHLY
    if os.path.exists('DATA/NSIDC0051_SEAICE_PS_N25km_'+str((ymax-1))+'06_v2.0.nc') == False:
        os.system("sed -i '' 's/YMAX/"+str((ymax-1))+"/g' nsidc_download_monthly.py")
        os.system("python nsidc_download_monthly.py")
        while os.path.exists('NSIDC0051_SEAICE_PS_N25km_'+str((ymax-1))+'06_v2.0.nc')==False:
            pass
        os.system('rm NSIDC0051*.xml')
        os.system('mv NSIDC0051*.nc DATA')
        os.system("sed -i '' 's/"+str((ymax-1))+"-06-30/YMAX-06-30/g' nsidc_download_monthly.py")
    ...
    for k,year in enumerate(range(1979,ymax+1)):
        ...
        if year == ymax:
            ...
            data = xr.open_dataset('DATA/NSIDC0051_SEAICE_PS_N25km_'+str(year)+'06_v2.0.nc')[key].isel(time=0).to_numpy()
            data[data>1] = np.nan
