...
datapath="/Users/williamgregory/Documents/PhD/CN_analysis/"

latf = "/Users/williamgregory/Documents/PhD/Datasets/GRIDS/psn25lats_v3.dat"
lat = np.fromfile(latf,dtype='<i4')
lat = lat.reshape(448,304)
lat = lat/100000

lonf = "/Users/williamgregory/Documents/PhD/Datasets/GRIDS/psn25lons_v3.dat"
    ...
    def read_daily(self, month, days, ymax):
            ...
            for y in range(2017,ymax+1):
                ...
                for d in range(1,days+1):
                    k = k + 1
                    icefile = open(glob.glob("/Users/williamgregory/Documents/PhD/Datasets/SIC/NSIDC_GSFC_daily_SIC/"+str(y)+"/nt_"+str(y)+str(month)+str("%02d"%d)+"*.bin")[0], "rb")
...
class SIC_monthly:
    ...
    def read_monthly(self, month):
        ...
        for y in range(1979,2016+1):
            icefile = open(glob.glob("/Users/williamgregory/Documents/PhD/Datasets/SIC/NSIDC_monthly_SIC/nt_"+str(y)+str(month)+"*.bin")[0], "rb")
            contents = icefile.read()
            icefile.close()
            s="%dB" % (int(dimX*dimY),)
            z=struct.unpack_from(s, contents, offset = hdr)
    ...
    def gen_networks(self, month, lats):
        ...
        for yend in range(2018,2018+1): #these are the forecast years
            ...
            CN.Network.cell_level(network, self.dt[:,:,range(nmax+1)], str(month), "_100sqkm_79-"+str(yend), datapath)
            CN.Network.tau(network, self.dt[:,:,range(nmax+1)], str(month), 0.01, "_100sqkm_79-"+str(yend), datapath)
            ...
            for i,j in itertools.product(range(dimX),range(dimY)):
                if (all(~np.isnan(target.dt[i,j,range(nmax)]))) & (max(abs(target.dt[i,j,range(nmax)])) > 0):
                    ...
                    for it in range(iterations):
                        theta_sig[it] = np.random.uniform(0.0001,1000)
                        theta_l[it] = np.random.uniform(0.0001,1000)
                        try:
